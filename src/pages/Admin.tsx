...
type RegionFormState = {
  ...
  description: string;
  imageUrl: string;
  id?: string; // For editing existing regions
};
...
const Admin = () => {
  ...
  const startEdit = (product: any) => {
    ...
    setProductForm({
      ...
      regionId: product.region?._id || product.region || '',
  ...
  const fetchRegions = async () => {
    try {
      const url = `${ENDPOINTS.regions}?v=${Date.now()}`;
      const data = await apiFetch<any[]>(url);
      const arr = Array.isArray(data) ? data : (Array.isArray((data as any)?.data) ? (data as any).data : []);
      setRegions(arr);
    } catch (e: any) {
      console.warn('Failed to load regions', e?.message || e);
      setRegions([]);
    }
  };
  ...
  const deleteRegion = async (id: string) => {
    const ok = confirm('Delete this region?');
    if (!ok) return;
    try {
      await apiFetch(`${ENDPOINTS.regions}/${id}`, { method: 'DELETE' });
  ...
  const handleEditRegionSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingRegion || !regionForm.name.trim()) {
      toast.error('Region name is required');
      return;
    }
    ...
    try {
      ...
      await apiFetch(`${ENDPOINTS.regions}/${regionForm.id}`, {
        method: 'PATCH',
        body: JSON.stringify(payload),
      });
        ...
        if (!Number.isNaN(price) && !Number.isNaN(stock) && price >= 0 && stock >= 0 && productForm.name.trim()) {
          try {
            ...
            const payload = {
              ...
              stock,
              quantityOptions: Array.isArray(productForm.quantityOptions) ? productForm.quantityOptions : [],
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage regions for products. These appear in product form.</p>
        </div>
      </div>
        ...
        <CardContent>
          <form onSubmit={addRegion} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-3">
              ...
              <ImageUploader
                images={regionForm.imageUrl ? [regionForm.imageUrl] : []}
                onUpload={async (files) => {
                  const uploadedUrls: string[] = [];
                  for (const file of files) {
                    try {
                  ...
                  setRegionForm(p => ({ ...p, imageUrl: uploadedUrls.length > 0 ? uploadedUrls[0] : '' }));
                  return uploadedUrls;
                }}
                isLoading={uploadingImage}
                maxImages={1}
      ...
      {/* Edit Region Dialog */}
      <Dialog open={!!editingRegion} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Make changes to your region here.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditRegionSubmit} className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="editRegionName" className="text-right">
                Name
              </Label>
              <Input
                id="editRegionName"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="editRegionDesc" className="text-right">
                Description
              </Label>
              <Textarea
                id="editRegionDesc"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="editRegionImage" className="text-right">
                Image
              </Label>
              <div className="col-span-3">
                <ImageUploader
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4">
              ...
              return (
                <Card key={rid}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold">{(r as any).name}</h3>
                        {(r as any).description && (
                          <p className="text-sm text-muted-foreground mt-1">{(r as any).description}</p>
                        )}
                        {r.imageUrl && (
                          <img src={r.imageUrl} alt={r.name} className="w-12 h-12 object-contain mt-2 rounded" />
                        )}
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => editRegion(r)}>
                          <Edit className="h-4 w-4 mr-1" /> Edit
                        </Button>
                        <Button size="icon" variant="destructive" onClick={() => deleteRegion(rid)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
            ...
            {regions.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-8">No regions found. Add your first region above.</p>
            )}
          </div>
        </CardContent>
      </Card>
